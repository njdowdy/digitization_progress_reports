...
def clean_import_tpt(df: pd.DataFrame):
    df_out = df.drop(df.index[0:3])
    df_out = df_out.drop(
        df_out.columns[[2, 3, 4, 5, 8, 11, 14, 17, 18, 19, 20, 21]], axis=1
    )
    df_out = df_out.set_axis(
        [
            ...
            "reported_hires_images",
        ],
        axis=1,
    )
    df_out = clean_import_template(df_out)
    return df_out


def make_totals_row(
    df: pd.DataFrame, total_label: str = "TOTAL", total_code: str = "TOTAL"
):
    df_out = df.append(df.sum(numeric_only=True), ignore_index=True)
    df_out.at[len(df_out) - 1, "collection_name"] = total_label
    df_out.at[len(df_out) - 1, "collection_code"] = total_code
    return df_out
...
def wide_to_long(df: pd.DataFrame):
    ...
    df_long = df_long[["collection_name", "collection_code", "class", "type", "value"]]
